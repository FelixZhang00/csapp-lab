...
int changeValue() {
  ...
  intPtr1 = intPtr1 + 5;
  *(unsigned char*)intPtr1 = 351&0x0ff;
  *((unsigned char*)intPtr1 + 1) = (351>>8)&0x0ff;
  return intArray[5];
}
...
int withinSameBlock(int * ptr1, int * ptr2) {
  //the first 26 bits of pointer is address,
  //the last 6 bits is the flag of the pointer	
  int p1 = (char*)&(*ptr1);
  int p2 = (char*)&(*ptr2);
  int mask = 0xFF;
  mask = (mask << 8)+0xFF;
...
int withinArray(int * intArray, int size, int * ptr) {
  int* beginPtr = intArray;
  int* endPtr = intArray + size;
  return (ptr>=beginPtr)&&(ptr < endPtr);
}
